...
export function LyceensDashboard({
  ...
  const handlePlanetSelect = (planet: Planet) => {
    router.push(`/lyceens/${childId}/module/${planet.slug}`);
  };
    ...
    <main className="relative min-h-screen overflow-hidden sky-bg">
      ...
      >
        <div className="mx-auto flex max-w-6xl items-center justify-between px-8 py-4">
          ...
          <p className="text-sm font-medium" style={{ color: 'var(--text-tertiary)' }}>
            Parcours <span style={{ color: 'var(--accent)' }}>Lycéens</span> · {firstName}
          </p>
        </div>
      </nav>
      ...
      <header className="relative z-20 mx-auto max-w-6xl px-8 pt-12 text-center">
        <p className="eyebrow reveal reveal-1">
          <span className="divider" />14 — 18 ans
        </p>
        ...
        <p
          ...
          style={{ color: 'var(--text-secondary)' }}
        >
          Six modules d&apos;entraînement. Choisis-en un ou passe au Cabinet pour
          un bilan d&apos;auto-évaluation.
        </p>
      </header>
      ...
      <div className="relative z-10 flex flex-col items-center justify-center pt-4">
        <Station
          ...
          cabinetHref={`/lyceens/${childId}/cabinet`}
        />
      </div>
    </main>
  );
